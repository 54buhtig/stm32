...
//		MOM=MOM+1;
	memset(text, 0, sizeof(text));
	sprintf(text,"\"Temperature\":{\"value\":%f},", temperature);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text,"\"Humidity\":{\"value\":%f}", humidity);
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	switch(type)
	{
		case MQTT_PKT_CMD:															//命令下发
			...
			if(result == 0)
			{
				Serial_Printf("cmdid: %s, req: %s, req_len: %d\r\n", cmdid_topic, req_payload, req_len);
				
				if(MQTT_PacketCmdResp(cmdid_topic, req_payload, &mqttPacket) == 0)	//命令回复组包
				{
					Serial_Printf("Tips:	Send CmdResp\r\n");
