...
uint8_t Speed2=100;     //速度


uint8_t Virtual_Key2 = 0;   //初始化虚拟按键为0，防止风扇打开
uint8_t Angle2=0;     //速度
//==========================================================
//	函数名称：	OneNet_DevLink
//
...
//		MOM=MOM+1;
	...
	memset(text, 0, sizeof(text));
	sprintf(text,"\"FAN\":{\"value\":%d},", Virtual_Key);
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text,"\"SERVO\":{\"value\":%d},", Virtual_Key2);
	strcat(buf, text);
		...
		case MQTT_PKT_PUBLISH:								//接收的Publish消息
			...
			{	
				...
				if(Fan_json != NULL)
				{
					...
					{
						Speed2 = Fan_json->valueint;
						if(Fan_json->valueint==1)
						{
							Virtual_Key=1;   //虚拟按键置为1
						}
					}
				}
				if(Servo_json != NULL)
				{
					...
					{
						Angle2 = Servo_json->valueint;
						if(Servo_json->valueint==1)
						{
							Virtual_Key2=1;   //虚拟按键置为1
						}
					}
				}
