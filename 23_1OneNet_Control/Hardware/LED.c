...
void LED_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1 |GPIO_Pin_8 ;   //引脚A1 和引脚A2
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA,&GPIO_InitStructure);
	
	
	GPIO_SetBits(GPIOA,GPIO_Pin_1|GPIO_Pin_8);  //默认关闭LED灯
}
...
void LED2_On(void)
{
		GPIO_ResetBits(GPIOA,GPIO_Pin_8 );  //打开LED1灯
}
...
void LED2_Off(void)
{
		GPIO_SetBits(GPIOA,GPIO_Pin_8 );  //关闭LED1灯
}
