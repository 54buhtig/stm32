...
#define AUTH_INFO	 "2018-10-31&res=products%2Fn9aSJm39Mh%2Fdevices%2Fmotor&et=1784415600&method=md5&sign=7OUZBYH1Dc5xoS%2F7iDBXSQ%3D%3D"//鉴权信息

#define DEVID		"motor"//设备ID

extern unsigned char esp8266_buf[128];
extern uint8_t SendString[2];
float Receive[5] = {10.1,11.1,12};
extern  float x;
...
//		MOM=MOM+1;
	memset(text, 0, sizeof(text));
	sprintf(text,"\"Temperature\":{\"value\":%f},", Receive[0]);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text,"\"Humidity\":{\"value\":%f}", Receive[1]);
	strcat(buf, text);
