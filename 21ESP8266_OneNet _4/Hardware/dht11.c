...
float temperature;   //用来发给esp8266的温度数据
float humidity;     //用来发给esp8266的湿度数据


float combine(int integer_part, int fractional_part) {
    int n = 0;
    int temp = fractional_part != 0 ? abs(fractional_part) : 0;
    ...
    // 组合整数和小数部分
    float fractional = (float)fractional_part / denominator;
...
void DHT11_REC_Data(void)
{
	...
	temperature = combine(TH,TL);
	humidity=combine(RH,RL);
}
