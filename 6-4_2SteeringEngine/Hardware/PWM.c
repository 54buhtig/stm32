...
void PWM_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11 ;   //引脚A11
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA,&GPIO_InitStructure);
	
	
	
	TIM_InternalClockConfig(TIM1);   //內部时钟作为定时器2的时钟,定时器上电默认使用内部时钟
	...
	TIM_TimeBaseInitStructure.TIM_Period = 20000-1;        //ARR值
	...
	TIM_OCInitStructure.TIM_Pulse = 0;      //CCR的值   2500~400
	
	TIM_OC4Init(TIM1,&TIM_OCInitStructure);     //通道4初始化
